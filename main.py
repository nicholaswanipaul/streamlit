...
with col2:
   ...
   # dataframe_price_year = price_data[new_category_year].groupby(['commodity', 'unit_price'])['Year'].sum()
    #dataframe_price_year = dataframe_price_year.reset_index()
     ...
     fig = px.scatter_mapbox(price_data, lat="latitude", lon="longitude", hover_name="admin2", hover_data=["admin1", "admin2"], color_discrete_sequence=["fuchsia"], zoom=3, height=300, use_container_width=True)
